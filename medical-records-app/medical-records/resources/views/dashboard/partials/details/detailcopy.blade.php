<div class="wrapper bg-white rounded-lg relative overflow-hidden py-3 print:py-0">
   ...
   @if ($prsc->prscPatient->patient_status == 'waiting' || $prsc->prscPatient->patient_status == 'finished')
   ...
   <input type="number" min="{{ $prsc->prscDetail->count() }}" value="1" class="mds-input print:hidden ml-9 w-60 px-4 my-2 py-2 rounded border-b-2 bg-gray-50 focus:bg-gray-200 border-b-blue-400 box-border focus:border-blue-500 hover:bg-gray-200 transition-colors outline-none placeholder:text-gray-500">
   ...
   <div class="pb-2">
      ...
      <div class="detail-container">
         ...
         {{-- <input type="hidden" name="prsc_number" value="{{ $prsc->patientPrsc->prsc_number }}"> --}}
         <input type="hidden" name="prsc_date" value="{{ now() }}">
         <?php $i = 0 ?>
         @if ($prsc->prscDetail->count() > 0)
            @foreach ($prsc->prscDetail as $detail)
               <div class="px-2 py-1 mds-card flex justify-start md:justify-evenly flex-wrap items-center {{ ($i%2 == 1)? "bg-gray-100" : "" }}">
                  ...
                  <a href="" onclick="return confirm('Remove {{ $detail->detailMedicine->medicine_name }} from medicine list?')" class="delete cursor-pointer print:hidden text-white inline-block px-2.5 py-0.5 rounded hover:bg-red-700 duration-200 bg-red-500 font-semibold" data-key="{{ $detail->id }}" title="delete medicine">X</a>

               </div>
               <?php $i++ ?>
            @endforeach
         @endif
         @if ($prsc->prscDetail->count() == 0 && $prsc->prscPatient->patient_status == 'consulting')
            <div class="px-2 py-1 mds-card flex justify-start md:justify-evenly flex-wrap items-center ">               
               <div class="my-1">                  
                  <label for="medicine_code" class="md:ml-2">Medicine : </label>                  
                  <input type="text" name="detail[{{ $i }}][medicine_code]" hidden id="medicine_code" class="mds-code w-44 px-4 rounded border-b-2 h-11 bg-gray-200 border-b-blue-400 placeholder:text-gray-500 outline-none">                  
                  <input type="text" class="mds-select w-44 px-4 rounded border-b-2 h-11 bg-gray-200 border-b-blue-400 placeholder:text-gray-500 outline-none">                  
               </div>                              
               <div class="my-1">                  
                  <label for="dose" class="md:ml-2">Dose : </label>                  
                  <input required type="text" class="w-36 px-4 rounded border-b-2 h-11 @error("dose") {{ "border-red-500" }} @else {{ "border-b-blue-400" }} @enderror bg-gray-50 focus:bg-gray-200 focus:border-2 focus:border-blue-400 border-b-blue-400 box-border hover:bg-gray-200 transition-colors outline-none placeholder:text-gray-500" name="detail[{{ $i }}][dose]">                  @error("dose")                  <label for="patient_number" class="text-red-500 text-sm ml-3">                     <span data-feather="alert-circle" class="text-center mb-1 w-4 h-4"></span>                     {{ $message }}                  </label>                  @enderror               </div>               <div class="my-1">                  <label for="pcs" class="md:ml-2">Pcs : </label>                  <input required type="number" class="mds-pcs w-16 pl-3 pr-1 rounded border-b-2 h-11 @error("pcs") {{ "border-red-500" }} @else {{ "border-b-blue-400" }} @enderror bg-gray-50 focus:bg-gray-200 focus:border-2 focus:border-blue-400 border-b-blue-400 box-border hover:bg-gray-200 transition-colors outline-none placeholder:text-gray-500" name="detail[{{ $i }}][pcs]" min="1" value="1">                  
                     @error("dose")                  
                        <label for="patient_number" class="text-red-500 text-sm ml-3">                     
                           <span data-feather="alert-circle" class="text-center mb-1 w-4 h-4"></span>                     
                           {{ $message }}                  
                  ...
                  <div class="my-1">                  
                     <label for="sub_total" class="md:ml-2">Sub Total : </label>                  
                     <input readonly type="text" class="sub-total w-44 px-4 rounded border-b-2 h-11 bg-gray-200 border-b-blue-400 placeholder:text-gray-500 outline-none" name="detail[{{ $i }}][sub_total]">               
                  </div>               
                  <button type="button" class="remove cursor-pointer text-white inline-block px-2.5 py-0.5 rounded hover:bg-yellow-700 duration-200 bg-yellow-500 font-semibold" title="remove medicine">X</button>            
               </div>
            <?php $i++ ?>
         @endif
         @if ($prsc->prscDetail->count() == 0 && ($prsc->prscPatient->patient_status == "waiting" || $prsc->prscPatient->patient_status == "finished"))
            <div class="h-11 w-8/12 flex justify-between items-center bg-blue-300/60 border-blue-500 rounded-lg border  px-3 mx-auto">
               <span>
                  No medicine yet
               </span>
      ...
      <div class="my-1 flex">
         ...
         <div class="input-group ml-auto">
            <label>Total : </label>
            <input type="text" id="total" readonly class="total ml-auto w-44 px-4 rounded border-b-2 h-10 bg-gray-200 border-b-blue-400 placeholder:text-gray-500 outline-none" name="total_payment" value="{{ $prsc->total_payment }}">
         </div>
      </div>
      </form>
   </div>
...
<script>
   $(document).ready(function() {
      ...
      // detail script
      let loop = {!! $i !!}
      let validTotal = '{!! ($prsc->total_payment)?? "0" !!}'
      let total = $('.total')
      validate()
